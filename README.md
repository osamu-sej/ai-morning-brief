...
毎朝5:00に`launchd`が前日分を収集・整理し、5:09にGoogle Chromeを自動起動、5:10にChrome拡張機能が新規Notebookを作成します。選定した最大30件を**1記事ずつ別々のテキストソース**として追加後、標準の音声解説生成を依頼します。10分の間隔で、Gemma処理完了前に古い資料を読む競合を防ぎます。Macがスリープしていないことが実行条件です。音声の完成時間はGemini Notebook側の処理状況に依存します。
